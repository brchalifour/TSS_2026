...
### Author: Bridget Chalifour, Ph.D. adapted from Fierer Lab, CU Boulder, GitHub Tutorial ####
### For TSS 2026 ####
...
# Setup
## Note the code below uses the example dataset from mctoolsR (https://github.com/leffj/mctoolsr). Advanced users -- We will note where you can change the inputs and code to use your own data instead.

## Learn about microbiomes first! Watch these short videos:
# https://www.youtube.com/watch?v=YqFCXpA7O3k What is a microbiome?

# Learn about microbes helping animals ---
# https://www.youtube.com/watch?v=8GPEs5ldBmc The Stinky Shield of the Hoopoe Bird
# https://www.youtube.com/watch?v=k4RQX1ci5TY&list=PLWYyvE5z4B2iXmXiBBsnTBzvTegv2Nbvz&index=2 How eating poop is key to Koalas' Survival
# https://www.youtube.com/watch?v=RJxi8d4sQd4&list=PLWYyvE5z4B2iXmXiBBsnTBzvTegv2Nbvz&index=4 The cat parasite that tricks rat brains


```{r}

# As we've learned previously, packages are pre-built to help you with specific tasks. 
# There are a few options when it comes to microbiome data, from a package literally called "microbiome", to "phyloseq" to "mctoolsr", which we will use here.
# The vegan package is also useful, it has all the complicated community ecology stats equations built into it so I don't have to write the formulas myself. Yay!
# Working with microbiome data is like working with community ecology data, just on a bigger scale, ironically. 
# Here we will install and load the needed packages for this tutorial first.
# Remember, to run lines, we make sure our cursor is on the line we want to run, then either hit "Run" with the green arrow in the top right,
# or CNTRL + ENTER for Windows.
...
```{r}
# We need to bring in some data to start practicing with! This dataset is built into the mctoolsr package.
# This is example dataset taken from a study examining the bacterial communities associated with fruits and vegetables.

# We're going to load in the data in steps, there will be three parts to our data. First, is the OTU table and taxonomy.
# An OTU stands for an operational taxonomic unit.
# Microbes are hard to classify as species, especially when they can't be cultured and grow in a lab.
# For this reason, we use their DNA sequence to define different species, but we don't call them species.
# We set a threshold of how close the DNA sequence can match (often ranging from 97% to 100%) to call the sequence a certain microbe.
# We match the DNA to a database to get the taxonomy, which is exactly like the taxonomy we use on other living things.
# From biggest group to smallest, we say:
# Domain
  # Kingdom
    # Phylum 
     # Class
       # Order
        # Family
          # Genus
            # Species (OTU, in this case)
# NOTE: Scientists are moving towards only using ASVs, which are the exact amplicon sequence variants.
# In this case, if one letter of DNA is different, we can the microbes different species (ASVs)
# You might see ASV or OTU in this tutorial, but they mean the same thing!

# Loading OTU Table with taxonomy
tax_table_fp = system.file('extdata', 'fruits_veggies_taxa_table_wTax.txt', 
                           package = 'mctoolsr')

# The above data just tells us how many sequencing reads came back in each sample, and what microbes they are.
# We now need to bring in metadata.
# Metadata matches the sample IDs to the information we have about them.
# In this case, we have information about the type of fruits/veggies, the farm they came from, etc.
...
```{r}
# Let's take a peek at the data to see the format, and generally what it looks like.
# Using the head() command is kind of like a sneak preview, it only shows the first few lines of data.
...
## This is very helpful to see the depth of sequencing, and make sure the samples that should have data, do have data.
## Some samples, like a BLANK sample, with no DNA in it, should have close to 0 reads.
```{r}
# This is done by getting the column sums of the sequence table, and we'll sort it too.
sort(colSums(input$data_loaded))
# Note that for microbiome data, especially from certain kinds of samples like fecal and soil, these are LOW numbers!
# For other kinds of data, like coming from an infant, or like a nasal swab, certain kinds of water, and certain kinds of built environment samples, these are on par.
# We often see totals in the tens or hundreds of thousands, but we are keeping it simple for this tutorial.
...
seqcounts <- as.data.frame(sort(colSums(input$data_loaded))) %>%
  dplyr::rename("seqs" = "sort(colSums(input$data_loaded))") %>%
  rownames_to_column(var = "sampleID")

# Remember when we learned ggplot??? Time to use it again!!
...
# Experienced users, see below. TSS, skip to next section:
...
# ALPHA DIVERSITY
# Alpha diversity refers to the differences within a community.
# We can think of microbiomes like little forests.
# In a forest, we can count the number of different tree species (this is the RICHNESS)
# We can look at how evenly distributed the species are (this is the EVENNESS)
# We can combine these measures and look at overall DIVERSITY (Shannon diversity index)
# Watch this short video first: https://www.youtube.com/watch?v=OtbifsDOgMw Alpha, Beta, and Gamma Diversity
...
```{r}
# Here is where we start using functions from the vegan package!
...
# Now we get into statistics! Statistics is the math biologists use to test for SIGNIFICANT differences.
# If you are interested in becoming any kind of scientist, statistics will become important!! Take a class!!
# Watch this short video first: https://www.youtube.com/watch?v=nb75_GFPD9A
# This one is longer but better: https://www.youtube.com/watch?v=QZ7kgmhdIwA

```{r}
# We're going to test differences in the alpha diversity of microbiomes between two categories, by farm type (conventional or organic)
# Two categories example

# First, we have to test assumptions to make sure we run the correct tests for our data. Run the lines below and make sure you get the same results:
leveneTest(input_filt$map_loaded$rich ~ input_filt$map_loaded$Farm_type)
# Variance homogeneous (p > 0.05)

shapiro.test(input_filt$map_loaded$rich)
# Richness normally distributed (p > 0.05)

t.test(input_filt$map_loaded$rich ~ input_filt$map_loaded$Farm_type)
# No significant difference in richness among the two categories p > 0.05

# If Levene's Test or Shapiro-Wilk Test p < 0.05, use Wilcoxon Test
wilcox.test(input_filt$map_loaded$rich ~ input_filt$map_loaded$Farm_type)
# Same result

# Back to ggplot because it's often easier to visualize results, rather than just see the stats.
# Box and whisker plot with points
ggplot(input_filt$map_loaded, aes(Farm_type, rich, colour = Farm_type)) +
...
# Change colors to be colorblind friendly (e.g., red and blue is better than red and green) ( YOU CAN CHANGE TO WHATEVER YOU WANT THOUGH)
ggplot(input_filt$map_loaded, aes(Farm_type, rich, colour = Farm_type)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(size = 3, alpha = 0.5) +
  labs(x = "Farm Type", y = "ASV Richness", colour = "Farm Type") +
...
ggplot(input_filt$map_loaded, aes(Farm_type, shannon, colour = Farm_type)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(size = 3, alpha = 0.5) +
...
# Which group has the higher average diversity?
```

## Three + categores: ANOVA (parametric) or Kruskal-Wallis (non-parametric)
# Now we're going to do very similar tests, the only difference is that now we have three or more groups to compare instead of two.
# In this case we are comparing the microbiomes of mushrooms, spinach, and strawberries.

```{r}
# Run these lines and make sure you get the same results!
# Three category example
leveneTest(input_filt$map_loaded$rich ~ input_filt$map_loaded$Sample_type)
# Variance homogeneous (p > 0.05)

# Run an ANOVA test
m <- aov(input_filt$map_loaded$rich ~ input_filt$map_loaded$Sample_type)
...
summary(m)
# Is this a significant effect? Look at the p-value and see if it is less than (significant) or greater than (non-significant) 0.05.
...
kruskal.test(input_filt$map_loaded$rich ~ input_filt$map_loaded$Sample_type)

# Significant effect. (agrees with ANOVA). Run posthoc.
kwAllPairsNemenyiTest(input_filt$map_loaded$rich ~ as.factor(input_filt$map_loaded$Sample_type))
# Similar result, but spinach/mushroom different now only marginal. 

# Use ANOVA result if assumptions passed (levene test and shapiro test above) because there is more power and data aren't rank transformed, which is a loss of information.
# SO, which test result should we use? ANOVA or Kruskal-Wallis?

# Plot. Just copy the previous plot, change the variable name and the colour argument! 
# Here we'll try the viridis palette.
ggplot(input_filt$map_loaded, aes(Sample_type, rich, colour = Sample_type)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(size = 3, alpha = 0.5) +
...
# I actually don't like the viridis palette because the yellow is hard to see.
# I also like to make colors intuitive to what they show, for example, making strawberry a pink color.
# Feel free to change these to what you like best!
...
ggplot(input_filt$map_loaded, aes(Sample_type, rich, colour = Sample_type)) +
  ...
  labs(x = "Sample Type", y = "ASV Richness", colour = "Farm Type") +
  scale_color_manual(values = c("#c29574", "#8eaf92", "#F58F9a")) +
  theme_bw() +
  theme(axis.title = element_text(face = "bold", size = 16), 
        axis.text = element_text(size = 14),
        legend.position = "none")
...
# Multipanel plot - combining lots of results into one plot.
...
m1 <- aov(rich ~ Sample_type, data = input_filt$map_loaded)
t1 <- emmeans(object = m1, specs = "Sample_type") %>%
  cld(object = ., adjust = "Tukey", Letters = letters, alpha = 0.05) %>%
  mutate(name = "rich",
         y = 180)

m2 <- aov(shannon ~ Sample_type, data = input_filt$map_loaded)
...
# because one is in the hundreds and the other goes up to 5.
...
plot_ordination(input_filt, ord, 'Sample_type', 'Farm_type', hulls = TRUE)
#this is a nice thing to do at the start of an analysis to see what your data is doing

# Note that we can use both colors and point shapes to differentiate groups!
...
input_filt$map_loaded$Axis01 <- scores(pcoa)[,1]
input_filt$map_loaded$Axis02 <- scores(pcoa)[,2]

# Function for making a convex hull
find_hull <- function(df) df[chull(df$Axis01, df$Axis02),]

# Calculate hulls and save to dataframe
micro.hulls <- ddply(input_filt$map_loaded, c("Sample_type", "Farm_type"), find_hull)

# Plot
ggplot(input_filt$map_loaded, aes(Axis01, Axis02, colour = Sample_type, shape = Farm_type)) +
...
### Huh??? This basically just means we run stats to test if whole microbiomes are different from each other, based on their category.

```{r}
# PERMANOVA (multivariate version of ANOVA)
set.seed(1223) # To make reproducible 
m <- adonis2(dm ~ input_filt$map_loaded$Sample_type*input_filt$map_loaded$Farm_type)
m
...
input_filt$map_loaded$Sample_type <- as.factor(input_filt$map_loaded$Sample_type)
set.seed(1223)
m_pw <- calc_pairwise_permanovas(dm, input_filt$map_loaded, "Sample_type")
m_pw # Each group is significantly different from the other.

# Let's replot the ordination and customize it.
plot_ordination(input_filt, ord, color_cat = "Sample_type", shape_cat = "Farm_type", hulls = TRUE) + 
  scale_color_manual(values = c("#c29574", "#8eaf92", "#F58F9a"), name = "Sample Type") +
  scale_fill_manual(values = c("#c29574", "#8eaf92", "#F58F9a"), name = "Sample Type") +
  labs(shape = "Farm Type") +
  theme(
  ...
  theme(legend.position = "right",
        ...
        axis.text = element_text(size = 14))
```
## TSS!!!! Skip to OTU/ASV Specific functions
...
```{r}
# This will make something like a phylogenetic tree based on how similar samples are to each other
# Kind of helpful to see how things are "related"
...
sim <- simper(t(input_filt$data_loaded), 
              input_filt$map_loaded$Sample_type)
s <- summary(sim)
...
mp <- multipatt(t(input_filt$data_loaded), 
                input_filt$map_loaded$Sample_type, 
                func = "IndVal.g", 
...
input_filt$taxonomy_loaded$taxonomy8 <- rownames(input_filt$taxonomy_loaded)

# It takes a summarized taxonomy dataframe, made like this.
tax_sum_asv <- summarize_taxonomy(input_filt, level = 8, report_higher_tax = FALSE)
...
return_top_taxa(input = input_filt, number_taxa = 10)
# Calculates the top taxa based on number of counts - taking the row means for each OTU

# What taxa are common? Give them rarefied input, then level = "what level we want to summarize our information for". Check input_filt$taxonomy_loaded for the levels. level 2 is phylum level
tax_sum_phyla <- summarize_taxonomy(input_filt, level = 2, report_higher_tax = FALSE)
...
# Check the most abundant phyla across dataset
tax_sum_phyla_Means <- sort(rowMeans(tax_sum_phyla), decreasing = T)

# This is now sorted from most abundant to least abundant, based on means across all samples
# Check the top 5 phyla
tax_sum_phyla_Means[1:5]

# Now lets try summarizing at the family level (level 5)
tax_sum_families <- summarize_taxonomy(input_filt, level = 5, report_higher_tax = FALSE)
...
# cliffplot_taxa_bars(input = input_filt, level = 1, variable = "Sample_type")
# cliffplot_taxa_bars(input = input_filt, level = 2, variable = "Sample_type")
# cliffplot_taxa_bars(input = input_filt, level = 3, variable = "Sample_type")
# cliffplot_taxa_bars(input = input_filt, level = 4, variable = "Sample_type")
# cliffplot_taxa_bars(input = input_filt, level = 5, variable = "Sample_type")
# cliffplot_taxa_bars(input = input_filt, level = 6, variable = "Sample_type")
# cliffplot_taxa_bars(input = input_filt, level = 7, variable = "Sample_type")
# cliffplot_taxa_bars(input = input_filt, level = 8, variable = "Sample_type")
...
bars <- plot_taxa_bars(tax_sum_families,
  ...
  left_join(., input_filt$map_loaded, by = c("group_by" = "sampleID"))
